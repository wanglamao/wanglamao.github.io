<!-- # PrivateLoRA for Efficient LLM Privatization -->
...
<!-- 读书与写字的重要性从二十一世纪的视角已无需赘述。AI已经创造 -->
<!-- 从成本-效益的角度来看，只有少数头部需求值得大公司花大价钱构建AI系统，剩下的需求便成为了不会被解决的“长尾需求”。 -->
<!-- 从成本-效益的角度来看，构建完整的AI系统成本都极其高昂，但是能创造的效益却天差地别，因此只有少数头部需求会被实际解决，剩下的需求便成为了不会被解决的“长尾需求”。 -->
<!-- 背后的原因可以很简单地通过成本-价值来解释。 -->
<!-- 完整的AI系统研发和运营成本都极其高昂，意味着必须有足够的市场规模才能够支持这些高科技。
在实现AGI之前，完成不同的任务需要构建不同的AI系统。 -->
<figure>
...
<!-- 以上两个问题反映了隐私和效率这组经典矛盾在生成式AI普及路上给人们设下的挑战。 -->
<!-- 普通用户无法私有模型，隐私和的局限让个人无法像享受私家车或者个人电脑那样去拥抱大语言模型。 -->
<!-- 带来的隐私保障和个人体验。获得千人一面的生成内容的同时又要担心隐私泄漏。 -->
...
<!-- 
AI模型私有化带来的隐私性和个人化才能让用户轻松自然地使用AI模型。



PrivateLoRA利用了参数高效微调(PEFT)的特性，将大语言模型分为私有参数和公共参数：公共参数指模型主干部分，部署于云上；私有参数指PEFT参数和明文处理相关参数，部署于个人终端。公共参数和云上算力整体作为公共基础设施，私有参数
在这样的架构下，端云间只传输人类不可读的激活值，所有涉及隐私的计算在用户个人终端上完成，实现对个人隐私的保护；PEFT参数承载的个人化内容叠加在公用的模型主干上，能实现AI模型的千人千面。
PrivateLoRA实现了对网络通讯和计算任务分配的极致优化，在手机上能达到与GPU上运行相近的处理速度，且速度损耗随模型尺寸增大而减少。相比传统方案只能运行低精度量化的七十亿参数模型，PrivateLoRA能以高精度的千亿参数甚至更大的模型为后端。
因此，凭借手机和常规5G网络，用户就能享受到流畅且高质量的生成。极致的性价比是PrivateLoRA推动生成式AI普及的杀手锏。 -->
<!-- 由于用户原始数据和私有参数自始至终都在个人设备上， -->
<!-- 由此，PrivateLoRA不在用户终端上运行完整模型也能让用户获得私有化模型上才有的私密性和定制性。 -->
<!-- 
PrivateLoRA
因此，在PrivateLoRA的加持下，终端用户无需昂贵的硬件设备就可以享受生成式AI私有化带来的隐私性、个性化和强劲性能。 -->
<!-- PrivateLoRA -->
<!-- 私有化产品的最大优势是（1）使用便捷和（2）无后顾之忧。 -->

<!-- 但是，AI的私有化是从未有过的。
在当前AI的语境下，用户利用了大科技公司提供的，似乎只有大科技公司为用户更加个性化的服务，
为了充分发挥大语言模型的潜能，必须在保护隐私的前提下，提供更贴合用户的体验。 -->
<!-- 大语言模型的私有化也同样有成本高昂的问题。

隐私和效率的矛盾亘古有之，在大语言模型的服务上也不例外。
假如不考虑自建GPU集群，终端用户获取大模型服务的方式不外乎纯云模式和纯端模式。
纯云模式性能有余隐私保护不足，纯端模式正相反，所以两种模式都无法支持终端用户敞开怀抱拥抱AI。
一种全新的大模型私有化方案亟待提出。
<!-- 大语言模型的涌现能力与参数规模息息相关，参数规模又对硬件规格提出要求。 -->
<!-- 因此，我们根据大语言模型运行时简单地将现有的方案分为纯云方案和纯端方案。 -->

<!-- AI是数据驱动的，不仅体现在训练AI模型需要大量数据，也体现在AI服务在大数据场景下才有意义。在生成式AI之前，大数据场景的构建者是服务提供商，而不是具体的个人。在推荐系统中，用户
但是生成式AI通过涵盖世界知识的模型将不同的用户需求进行了统一 -->
...
<!-- 私有参数量少，承载个人化信息和负责明文数据的编解码；公共参数量大，使用强力的云端资源 -->
<!-- 
LoRA等PEFT方法通过引入极少量额外参数来推动庞大的模型主干实现下游任务适配，而且任务专有参数和主干参数在形式上是分开的。
受此启发，我们认为可以通过PEFT方式分离私有参数和公有参数，私有参数量虽然很少，但足以支持驱动模型私有化。 -->
...
<!-- ![PrivateLoRA](overview.gif) -->
<!-- 由于隐私制约，深度定制的大语言模型体验 -->

<!-- ## Introduction
大语言模型成为了用户与计算机世界的新桥梁。大语言模型的需求水涨船高。

当前大模型服务范式不外有二，纯云的模式和纯端的模式。
纯云模式发挥其云上充足的算力来驱动大尺寸模型提供高质量的生成。但是在纯云模式下，用户的请求和生成内容都在云上留下痕迹，使得终端用户拥抱AI时心存芥蒂。
对于服务商来说，隐私保护的种种制约转化为技术层面的不可逾越的阻碍，阻止服务商提供更加贴合用户需求的服务。

与纯云模式对应的是纯端的模式，即在个人设备上运行大语言模型，社区在这方面也已取得长足的进步。例如，MLC Chat能在智能手机上运行7B以下的模型，llamacpp能在笔记本电脑、树莓派等设备上运行。
但是由于硬件局限，纯端模式能够运行的模型尺寸很难让人满意，而模型尺寸是大模型展现涌现能力的核心条件。 -->

## 三、为什么PrivateLoRA可行？
<!-- ## Why is PrivateLoRA possible? -->
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
...
理论上，PrivateLoRA可以对所有线性模块做适配，但是为了最小化传输量，我们只选择自注意力模块的qkv做适配。transformer模型的生成是迭代式采样过程，每生成一个token需要做一次模型前馈。整个生成过程可以分为预填(Prefill)和解码(Decode)阶段。预填是对query进行第一次前馈的过程，主要用于计算kv缓存和生成第一个token；解码则是利用预填算好的kv缓存来进行迭代式采样。预填阶段所有线性模块的输入维度都是$bs\times seq\_len \times hidden$。而解码阶段由于kv缓存的存在，qkv的输入维度都是$bs\times 1\times hidden$，与序列长度无关，但是其他线性模块（out, mlp里的线性）都与序列长度有关。 -->
...
<!-- 由于搭建完整的GPU-手机推理框架进行实测十分困难，我们先对生成速度进行预估。 -->
<!-- 
PrivateLoRA相比在GPU/手机上运行完整模型增加了部分overhead。忽略了Word Embedding的耗时，前馈一次的时间可以分解为：
$$T=T^{Decoder}_{C}+T^{PL}+T^{LM\ Head}_{D},$$
其中$T^{Decoder}_{C}$为解码器栈在云端运行的时间，$T^{LM\ Head}_{D}$为手机上运行LM Head的时间，$T^{PL}$为PrivateLoRA引入的总耗时。

$T^{PL}$可以继续细分为云上$A,B$计算耗时$T^{PL}_D$、端上$M$计算耗时$T^{PL}_C$和网络传输耗时$T^{PL}_{Network}$
$$T^{PL}=T^{PL}_D + T^{PL}_C+T^{PL}_{Network},$$ -->
...
<!-- 演讲的最后，吴恩达老师指出“AI发展的下一个时代主题是AI普及”，。

私有化全尺寸大语言模型代价高昂，但是模型私有化又是深化AI赋能绕不过去的事。 -->
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
